...
uint8_t SpiRead(uint8_t address, uint8_t size){
	...
	HAL_SPI_TransmitReceive_DMA(&hspi1, &address, SpiRxBuffer, 1);
  while (!(SPI1->SR & SPI_FLAG_TXE)); // check if transmit buffer has been shifted out
	while ((SPI1->SR & SPI_FLAG_BSY));
	// for multi-read mode
	for(int i = 0; i < size; i++){
		HAL_SPI_TransmitReceive_DMA(&hspi1, &a, &SpiRxBuffer[i], 1);
	
	  while (!(SPI1->SR & SPI_FLAG_TXE)); // check if transmit buffer has been shifted out
		while ((SPI1->SR & SPI_FLAG_BSY)); // shouldnt be here
	}
	while ((SPI1->SR & SPI_FLAG_BSY)); // check BUSY flag
...
void SpiWrite(uint8_t address, uint8_t value){
  ...
  while (!(SPI1->SR & SPI_FLAG_TXE)); // check if transmit buffer has been shifted out
	while ((SPI1->SR & SPI_FLAG_BSY));
	HAL_SPI_TransmitReceive_DMA(&hspi1, &value, SpiRxBuffer, 1);
...
// raw-cal = 0
// get dt
/* For better precision you can use timer to get time between two readings, later used in integration*/
